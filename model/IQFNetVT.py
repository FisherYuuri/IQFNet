...
class ISQAM(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(ISQAM, self).__init__()
...
class IQFM(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(IQFM, self).__init__()
...
class IQFNet(nn.Module):
    def __init__(self,pretrained=False):
        super(IQFNet, self).__init__()
        ...
        self.eqm1 = ISQAM(in_channels=128, out_channels=64)
        self.eqm2 = ISQAM(in_channels=256, out_channels=128)
        self.eqm3 = ISQAM(in_channels=512, out_channels=256)
        self.eqm4 = ISQAM(in_channels=1024, out_channels=512)

        self.iqm1 = IQFM(in_channels=64, out_channels=64)
        self.iqm2 = IQFM(in_channels=128, out_channels=128)
        self.iqm3 = IQFM(in_channels=256, out_channels=256)
        self.iqm4 = IQFM(in_channels=512, out_channels=512)
    ...
    def forward(self, rgb, t):
        ...
        x1_vdt = self.iqm1(x1e, x1s)
        x1_vdt = self.up2(x1_vdt)
        x2_vdt = self.iqm2(x2e, x2s)
        x2_vdt = self.up2(x2_vdt)
        x3_vdt = self.iqm3(x3e, x3s)
        x3_vdt = self.up2(x3_vdt)
        x4_vdt = self.iqm4(x4e, x4s)
        ...
        x_refine = self.refunet(x_refine)

        return x1e_pred, x1e_pred_t, x_pred, x_refine


    def load_pretrained_model(self):
        self.swin1.load_state_dict(torch.load('/media/kemove/403plus/yuhuan/IFENet-main/swin_base_patch4_window12_384_22k.pth')['model'],strict=False)
